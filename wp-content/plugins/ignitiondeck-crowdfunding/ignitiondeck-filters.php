...
function id_project_goal($goal, $post_id, $noformat = false) {
	if ($noformat) {
		...
		return apply_filters('id_display_currency', apply_filters('id_price_format', $goal, $post_id), $post_id);
	}
}
...
function id_postmeta_box_fields($fields) {
	...
	foreach ($fields as $field) {
		...
		if (isset($field['id'])) {
			$id = $field['id'];
			if ($id == 'ign_project_type' && $mode !== 'legacy') {
				$disabled[] = $i;
			}
			else if ($id == 'ign_end_type' && ($mode == 'legacy' || $mode == 'idc_free')) {
				$disabled[] = $i;
			}
		}
		$i++;
