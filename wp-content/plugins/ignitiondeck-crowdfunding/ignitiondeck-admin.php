...
function id_admin_menus() {
	...
	if (current_user_can('manage_options')) {
		$project_settings = add_menu_page( __('Project Settings', 'ignitiondeck'), __('IDCF', 'ignitiondeck'), 'manage_options', 'ignitiondeck', 'product_settings', plugins_url( '/images/ignitiondeck-menu.png', __FILE__ ));
		if (is_id_licensed()) {
...
function admin_menu_html() {
	...
	$menu = '
		<div class="sidebar ignitiondeck">
			...
			<div class="help">
				<a href="http://forums.ignitiondeck.com" alt="IgnitionDeck Support" title="IgnitionDeck Support" target="_blank"><button class="button button-large">'.__('Support', 'ignitiondeck').'</button></a>
				<a href="http://docs.ignitiondeck.com" alt="IgnitionDeck Documentation" title="IgnitionDeck Documentation" target="_blank"><button class="button button-large">'.__('Documentation', 'ignitiondeck').'</button></a>
			</div>
			<br style="clear: both;"/>
			<h3 class="nav-tab-wrapper">';
	$menu .= apply_filters('idcf_project_settings_tab', '<a '.(($_GET['page'] == "ignitiondeck") ? ' class="nav-tab nav-tab-active"' : ' class="nav-tab"').' href="admin.php?page=ignitiondeck">'.$tr_Product_Settings.'</a>');
	if (is_id_licensed()) {
	...
	if (is_id_licensed() && $platform == 'legacy') {
		$menu .= '<a '.(($_GET['page'] == "email-settings") ? ' class="nav-tab nav-tab-active"' : ' class="nav-tab"').' href="admin.php?page=email-settings">'.$tr_Email_List_Settings.'</a>';
	}	
	$menu_sub = '</h3></div>';
