jQuery(document).ready(function() {
	...
	// Hiding option for Fund type selection, when there is only one option.
	jQuery('[name="project_fund_type[]"]').each(function(index, element) {
		if (jQuery(this).children('option').length <= 1) {
			jQuery(this).parents('.form-row').hide();
		}
	});
	...
	jQuery('form#fes').submit(function(e) {
		...
		if (btnID == "project_fesubmit") {
			...
			if (!noError) {
				console.log(jQuery('.error').get(0));
	...
	var minLevels = parseInt(jQuery('input[name="project_levels"]').attr('min'));
	jQuery('#fes input[name="project_levels"]').change(function() {
		...
		if (parseInt(jQuery(this).val()) < minLevels) {
	...
	if (levelChange < 0) {
		...
		for (i = 1; i <= levelChange; i++) {
			...
			if (jQuery('#fes .form-level:hidden').length > 0) {
				...
				jQuery(clone).find('#project_level_1_long_description-html').attr('id', 'project_level_' + last + '_long_description-html').data('wp-editor-id', 'project_level_' + last + '_long_description').attr('data-wp-editor-id', 'project_level_' + last + '_long_description');
				jQuery(clone).find('#project_level_1_long_description-tmce').attr('id', 'project_level_' + last + '_long_description-tmce').data('wp-editor-id', 'project_level_' + last + '_long_description').attr('data-wp-editor-id', 'project_level_' + last + '_long_description');
				...
				jQuery(clone).find('#wp-project_level_1_long_description-editor-container').attr('id', 'wp-project_level_' + last + '_long_description-editor-container');
				...
				// Trigger that replacing ids complete
				jQuery(document).trigger('idcfLevelCloned', [clone, last]);
