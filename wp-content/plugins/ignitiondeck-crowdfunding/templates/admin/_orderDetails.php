...
<div class="wrap">
	...
	<p class="search-box">
		<label class="screen-reader-text" for="post-search-input"><?php _e('Search Orders', 'ignitiondeck'); ?>:</label>
		...
		if (count($items) > 0) {
			...
			for ($i=0; $i < count($item_array); $i++) {
				...
				<tr>
					...
					<td valign="top" class="alternatetd"><?php echo (isset($item_array[$i]->prod_price) && $item_array[$i]->prod_price > 0 ? apply_filters('id_display_currency', apply_filters('id_order_price', $post_id, $item_array[$i]), $item_array[$i]->prod_price, $post_id) : ''); ?></td>
