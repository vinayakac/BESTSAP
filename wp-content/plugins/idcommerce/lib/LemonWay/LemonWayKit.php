...
class LemonWayKit{	
	...
	private static $accessConfig = array (
							// 'directKitUrl' => 'https://ws.lemonway.fr/mb/test/dev/directkit/service.asmx',
							// 'webkitUrl' => 'https://m.lemonway.fr/mb/test/dev/',
							'directKitUrl' => 'https://ws.lemonway.fr/mb/demo/dev/directkit/service.asmx',
							'webkitUrl' => 'https://m.lemonway.fr/mb/demo/dev/',
							'wlLogin' => 'society',
							'wlPass' => '123456',
							'language' => 'en');

	function __construct($settings, $test = false, $language = 'en', $debug = false) {
		self::$printInputAndOutputXml = $debug;
		$accessConfig = array();
		if ($test) {
			$accessConfig['directKitUrl'] = $settings['lemonway_directkit_url_test'];
			$accessConfig['webkitUrl'] = $settings['lemonway_webkit_url_test'];
			$accessConfig['wlLogin'] = $settings['lemonway_login_test'];
			$accessConfig['wlPass'] = $settings['lemonway_pass_test'];
		} else {
			$accessConfig['directKitUrl'] = $settings['lemonway_directkit_url'];
			$accessConfig['webkitUrl'] = $settings['lemonway_webkit_url'];
			$accessConfig['wlLogin'] = $settings['lemonway_login'];
			$accessConfig['wlPass'] = $settings['lemonway_pass'];
		}
		$accessConfig['language'] = $language;
	...
	public function GetWalletDetails($params) {
		$res = self::sendRequest('GetWalletDetails', $params, '1.8');
		if (!isset($res->lwError)){
			$res->wallet = new Wallet($res->lwXml->WALLET);
		}
		return $res;
