...
class ID_Authorize_Net {
	...
	public function check_payment_profile_exists($fname, $lname, $email, $cc_number, $cc_expiry, $cc_code, $Fields) {
		...
		if ($profileResponse->isOk()) {
			...
			if (!$card_exists) {
				...
				$responsePayment = $request->createCustomerPaymentProfile($this->customer_profile_id, $customerPaymentProfile, apply_filters('idc_authnet_validationmode_select', 'none'));
	...
	public function create_charge_token($level_price) {
		...
		if ($response->isOk()) {
			...
			$errorResponse = $response->getTransactionResponse();
			print_r(json_encode(array('response' => __('failure', 'memberdeck'), 'message' => "Error: ".$errorResponse->response_reason_text." Code: ".$errorResponse->response_reason_code." (".$response->getMessageCode().")")));
			exit();
		}
	...
	public function update_payment_profile_address($fields, $address="", $city="", $state="", $zip_code="") {
		...
		if (isset($avs) && $avs) {
			...
			if ($storedPaymentProfile->isOk()) {
				...
				$addressResponse = $addressRequest->updateCustomerPaymentProfile($this->customer_profile_id, $this->customer_payment_profile_id, $paymentProfile, apply_filters('idc_authnet_validationmode_select', 'none'));
	...
	public function idc_authnet_add_address($customer_payment_profile, $fields, $line) {
		...
		if (isset($avs) && $avs) {
			...
			$address_1 = apply_filters('idc_authnet_address_1_filter', ((isset($idc_address_1)) ? $idc_address_1 : ''));
			$address_2 = apply_filters('idc_authnet_address_2_filter', ((isset($idc_address_2)) ? $idc_address_2 : ''));
			$address_city = apply_filters('idc_authnet_address_city_filter', ((isset($idc_address_city)) ? $idc_address_city : ''));
			$address_state = apply_filters('idc_authnet_address_state_filter', ((isset($idc_address_state)) ? $idc_address_state : ''));
			$address_zip_code = apply_filters('idc_authnet_address_zip_code_filter', ((isset($idc_address_zip_code)) ? $idc_address_zip_code : ''));
	...
	public function idc_authnet_validationmode_filter($validation_mode) {
		global $avs;
		if (!empty($avs) && $avs) {
