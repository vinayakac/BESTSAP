...
class MD_Keys {
	...
	function generate_license($user_id, $dl_id) {
		if (!empty($user_id)) {
			$user = get_userdata($user_id);
			$email = $user->user_email;
			$this->license = apply_filters('idc_generate_license', (sha1(mt_rand(10000,99999).time().$email)), $dl_id);
			return $this->license;
		}
	}
				...
				if (!empty($download->download_levels)) {
					...
					if (is_array($levels) && in_array($level_id, $levels)) {
						if ($download->licensed == 1) {
							$license = $key->generate_license($user_id, $dl_id);
							if (isset($license)) {
