...
class ID_Member {
	...
	public static function get_customer_data($cust_id) {
		...
		$res = $wpdb->get_row($sql);
		if (empty($res)) {
			// We did't find an entry with customer_id in members table, might be in connected accounts, so finding in user_meta
			$sql = $wpdb->prepare("SELECT * FROM ".$wpdb->prefix."usermeta WHERE meta_key LIKE 'customer_id_%%' AND meta_value = %s LIMIT 1", $cust_id);
			$res = $wpdb->get_row($sql);
		}
		return $res;
	}
